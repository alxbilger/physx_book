...
== Ordinary Differential Equation <sec_ode>
...
#definition(title: "Implicit first-order system of ODEs")[
  ...
  $ <eq_implicit_first_order_system_ode>
...
#definition(title:"Mass matrix ODE")[
  ...
  A mass matrix ODE is a special case of an implicit first-order system of ODEs (@eq_implicit_first_order_system_ode) of the form:
  ...
  where $odemassmatrix$ is coupling matrix called *mass matrix*.
]

#property(title:"Mass matrix is the identity")[
  In a mass matrix ODE, if the mass matrix is the identity ($odemassmatrix = identity$), then the ODE is a regular explicit first-order of the form $y' = f(t,y)$.
]

#property(title:"Invertible mass matrix")[
  In a mass matrix ODE (@eq_mass_matrix_ode), if the mass matrix is invertible, the ODE can be rewritten:

  $
    mat(delim: #("{", none),
      y'     &= odemassmatrix(t,y)^(-1) f(t,y);
      y(t_0) &= y_0
    )
  $

  This can be considered as a regular explicit first-order equation of the form $y'=g(t,y)$, where $g(t,y) = odemassmatrix(t,y)^(-1) f(t,y)$.
]
...
#definition(title:"Residual")[
  ...
  For an explicit first-order ODE (@eq_explicit_first_order_ode),
  define the residual function 
  $
    residualode(t, y, y') = y' - f(t,y)
  $

  For a mass matrix ODE (@eq_mass_matrix_ode),
  define the residual function 
  $
    residualode(t, y, y') = odemassmatrix(t,y) thick y' - f(t,y)
  $ <eq_residual_mass_matrix_ode>
]

#property(title:"Residual integration")[
  The continuous equations (@eq_explicit_first_order_ode and @eq_mass_matrix_ode) are
  $
    residualode(t, y, y') = 0", for all" t
  $

  Then,

  $
    integral_(t_n)^(t_(n+1)) residualode(t, y, y') thick dif t = 0
  $ <eq_integral_residual>

  This equation gives the relationships between the state at $t_(n+1)$ and the previous time steps. It allows to apply a time-stepping method.
]

#definition(title:"Quadrature operator")[
  Introduce a generic quadrature operator
  $quadratureoperator[phi]$
  that approximates
  $
    integral_(t_n)^(t_(n+1)) phi(t) thick dif t
  $
]

#property(title:"General integration of ODEs")[

  Given an ODE with a residual function $residualode$ and a quadrature operator $quadratureoperator$:
  $
    quadratureoperator[residualode(t,y,y')] = 0
  $ <eq_general_integration_ode>
]

Let's consider 3 cases in the mass matrix ODE (@eq_mass_matrix_ode):

1) The mass matrix is invertible

2) The mass matrix is constant

3) The mass matrix can be singular and not constant

#property(title:"Mass matrix is invertible")[
  For a mass matrix ODE (@eq_mass_matrix_ode), if the mass matrix is invertible, the ODE can be rewritten:
  ...
  $
    residualode(t, y, y') = y' - odemassmatrix(t,y)^(-1) f(t,y)
  $

  In that case, the integral of the residual is the difference of 2 terms: 
  $integral_(t_n)^(t_(n+1)) y' thick dif t $ and
  $integral_(t_n)^(t_(n+1)) odemassmatrix(t,y)^(-1) f(t,y) thick dif t$.

  By the *Fundamental Theorem of Calculus*,

  $
    integral_(t_n)^(t_(n+1)) y' thick dif t = y_(n+1) - y_n
  $

  By substitution:

  $
    integral_(t_n)^(t_(n+1)) residualode(t, y, y') thick dif t = y_(n+1) - y_n - integral_(t_n)^(t_(n+1)) odemassmatrix(t,y)^(-1) f(t,y) thick dif t
  $

  In @eq_integral_residual, $integral_(t_n)^(t_(n+1)) residualode(t, y, y') thick dif t = 0$, then,
  $
    y_(n+1) - y_n - integral_(t_n)^(t_(n+1)) odemassmatrix(t,y)^(-1) f(t,y) thick dif t = 0
  $ <eq_integral_residual_mass_matrix_ode_invertible>

  In @eq_integral_residual_mass_matrix_ode_invertible, the integral $integral_(t_n)^(t_(n+1)) odemassmatrix(t,y)^(-1) f(t,y) dif t$ can be approximated by a quadrature rule.
]

#property(title:"Mass matrix is constant")[
  For a mass matrix ODE (@eq_mass_matrix_ode), if the mass matrix is constant (but not necessarily invertible), the integral of the residual is the difference of 2 terms: 
  $integral_(t_n)^(t_(n+1)) odemassmatrix(t,y) y' thick dif t $ and
  $integral_(t_n)^(t_(n+1)) f(t,y) thick dif t$.

  Since $odemassmatrix$ is constant, it can be pulled out from the integral:

  $
    integral_(t_n)^(t_(n+1)) odemassmatrix(t,y) y' thick dif t =
    odemassmatrix integral_(t_n)^(t_(n+1))  y' thick dif t
  $
  ...
  $
    integral_(t_n)^(t_(n+1)) residualode(t, y, y') thick dif t = odemassmatrix [y_(n+1) - y_n] - integral_(t_n)^(t_(n+1)) f(t,y) thick dif t
  $
  ...
  In @eq_integrated_mass_ode, the integral $integral_(t_n)^(t_(n+1)) f(t,y) thick dif t$ can be approximated by a quadrature rule.
]

#property(title:"Mass matrix is singular and not-constant")[
  For a mass matrix ODE (@eq_mass_matrix_ode), if the mass matrix can be singular, and is not constant,

  $
    integral_(t_n)^(t_(n+1)) residualode(t, y, y') thick dif t = 0
    <=>
    integral_(t_n)^(t_(n+1)) (odemassmatrix(t,y) thick y' - f(t,y)) thick dif t = 0
  $
]

#property(title:"Integration of an explicit first-order ODE")[
  ...
  In @eq_integrated_ode, the integral can be approximated by a quadrature rule.
]
